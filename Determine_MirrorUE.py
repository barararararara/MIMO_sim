...
def process_single_link(channel_base,channel_type,d):
    ...
    R = distance_from_mirrorUE(d, tau, rho, N, M)
    ...
    return {
        ...
        "r_dash": r_dash, "R": R, "sca1_xyz_co": sca1_xyz_co,
        "subarray_v_qy_qz": subarray_v_qy_qz, "dis_sca1_to_anntena": dis_sca1_to_anntena
    }

if __name__ == '__main__':
    for d in distance_list:
        print(f"Processing distance: {d} meters")
        save_dir = f"C:/Users/tai20/Downloads/研究データ/Data/Mirror/{channel_type}/{NF_setting}/d={d}"
        os.makedirs(save_dir, exist_ok=True)
        with Pool() as pool:
            args = [(channel_base, channel_type, d)
                    for channel_base in Base.tolist()]
            result = pool.starmap(process_single_link, args)
        print(f"Saving results to {save_dir}/Scatter1.npy")
