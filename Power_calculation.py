...
def Power_calculation(Channel_scatter1, channel_type, d, f, start_idx, end_idx):
    P_sub_dash_dBm_list = []
    ...
    for number in range(start_idx, end_idx):
        ...
        phi_rad_v = np.radians(phi_deg_v)
        theta_rad_v = np.radians(theta_deg_v)
        print("converted phi_rad_v, theta_rad_v")
...
Ssub_lam = 0
d=30
Channel_Scatter1 = np.load(f"C:/Users/tai20/Downloads/sim_data/Data/{Method}/{channel_type}/Ssub={Ssub_lam}lam/Scatter1/d={d}/Scatter1.npy", allow_pickle=True)
for i in range(0,1): 
    Power_calculation(Channel_Scatter1, 'InH', d, f, i*100, (i+1)*100)
